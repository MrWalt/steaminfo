  ...
  return (
    <div className="flex flex-col col-start-1 col-end-4 row-start-2 row-end-4">
      <div className="flex text-primary-50">
        <span
          className="bg-accent-950 inline-block px-2 py-1 rounded-tl-sm hover:bg-accent-950 transition-colors cursor-pointer"
          onClick={handleShowPlaytime}
        >
          Playtime
        </span>
        <span
          className="bg-accent-800 inline-block px-2 py-1 rounded-tr-sm hover:bg-accent-950 transition-colors cursor-pointer"
      ...
      <div className="bg-primary-600 rounded-b-sm border-t-4 border-accent-950 rounded-tr-sm grid grid-cols-[repeat(3,_1fr)] grid-rows-[400px]">
        ...
        {currentTab === "inventory" && <div>Inventory</div>}
      </div>
    </div>
    // <>{showInventory ? <div>Inventory</div> : <Playtime steamId={steamId} />}</>
  );
}
